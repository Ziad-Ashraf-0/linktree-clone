<template>
    ...
    $emit('updatedInput', { id: link.id, str: 'isName' })
        " class="font-semibold mr-2 cursor-pointer" :class="isMobile ? 'text-xl' : 'text-sm'">
                        ...
                        <Icon @click="$emit('updatedInput', { id: link.id, str: 'isName' })" class="cursor-pointer"
            ...
            <div class="flex items-center justify-between py-1">
                <div class="flex items-center w-full">
                    <input v-if="editLink(selectedId, selectedStr)" :id="`editLinkInput-${link.id}`" type="text"
                        v-model="url" class="w-full text-sm focus:outline-none">
    ...
    $emit('updatedInput', { id: link.id, str: 'isLink' })
        ...
        { 'max-h-0 transition-all duration-200 ease-out': !isUploadImage },
            ...
            <div class="w-full flex items-center justify-between px-4 py-5">
                ...
                <div class="w-full pl-3">
                    <button @click="openCropper = true" class="
                            flex 
                            items-center 
                            justify-center 
                            w-full 
        ...
        <CropperModal v-if="openCropper" :linkId="link.id" @data="data = $event" @close="openCropper = false" />
    </div>
</template>
